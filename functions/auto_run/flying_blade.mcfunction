...
#前进 /                                                            前进速度 v
execute as @e[type=function:flying_blade] at @s if block ~~~ air run tp @s ^^^0.5 facing @e[type=function:knive_navigation,c=1]
...
#侦测冷却
scoreboard players remove @e[scores={version=3,tick2=1..}] tick2 1
scoreboard players add @a tick2 0
execute if score knive_cd tick2 matches ..0 run scoreboard players set knive_cd tick2 200


#太久没判断到实体或墙壁，就清除
scoreboard players add @e[type=function:flying_blade] tick 1
execute as @e[type=function:flying_blade,scores={tick=100..}] run kill @e[type=function:knive_navigation]
execute as @e[type=function:flying_blade,scores={tick=100..}] run kill @e[type=function:flying_blade]